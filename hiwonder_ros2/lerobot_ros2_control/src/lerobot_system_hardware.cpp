  ...
  const hardware_interface::HardwareComponentInterfaceParams & params)
{
  ...
  for (const auto & joint : info_.joints) {
    ...
    if (!calibration_joint) {
      RCLCPP_ERROR(get_logger(), "Calibration for joint '%s' is missing", joint.name.c_str());
      return CallbackReturn::ERROR;
    }

    if (joint.command_interfaces.size() != 1) {
      RCLCPP_ERROR(get_logger(), "Joint '%s' must have exactly one command interface", joint.name.c_str());
      return CallbackReturn::ERROR;
    }
    ...
    try {
      const auto servo_id = calibration_joint["servo_id"].as<unsigned int>();
      const auto lower_position = calibration_joint["lower_position"].as<unsigned int>();
      const auto upper_position = calibration_joint["upper_position"].as<unsigned int>();

      if (servo_id > std::numeric_limits<uint8_t>::max()) {
        throw std::out_of_range("servo_id exceeds uint8_t range");
      }
      if (lower_position > std::numeric_limits<uint16_t>::max() ||
          upper_position > std::numeric_limits<uint16_t>::max())
      {
        throw std::out_of_range("calibration position exceeds uint16_t range");
      }

      config.servo_id = static_cast<uint8_t>(servo_id);
    ...
    if (config.lower_position == config.upper_position) {
      RCLCPP_ERROR(get_logger(), "Calibration range for joint '%s' is zero", joint.name.c_str());
      return CallbackReturn::ERROR;
    }
